...
def randShowGenImage(x, y, imGenerator):
...
def loadData(classCount, batchSize):
    ...
    randShowGenImage(x_train, y_train, imGenerator) # random preview data augmentation effect
...
model = tf.keras.models.Sequential([
    # Conv 1
    layers.Conv2D(32, kernel_size = 3, padding='same', activation='relu', input_shape=(28,28,1), data_format='channels_last'),
...
import tensorflow as tf
filePath = './Models/MNIST_FCNN2_DA_0.9962.h5'
...
model = tf.keras.models.load_model('./Models/MNIST_CNN_0.9957.h5', compile=False)
loss_fn = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True)
opt_fn = tf.keras.optimizers.Adam(0.001)
model.compile(optimizer=opt_fn, loss=loss_fn, metrics=['accuracy'])
print(model.evaluate(x=ds_test, verbose=0))
