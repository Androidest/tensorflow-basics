...
def create_WGAN_GP(seed_size, gen_lr, dis_lr):
    generator = create_generator(seed_size)
    discriminator = create_discriminator()
    gen_opt = tf.keras.optimizers.Adam(learning_rate=gen_lr, beta_1=0.5, beta_2=0.9)
    dis_opt = tf.keras.optimizers.Adam(learning_rate=dis_lr, beta_1=0.5, beta_2=0.9)
    ...
    gan_model = WGAN_GP(discriminator, generator, seed_size)
...
gan_model = create_WGAN_GP(seed_size, gen_lr, dis_lr)
...
board_cb = tf.keras.callbacks.TensorBoard( log_dir='./tb_logs/') 
