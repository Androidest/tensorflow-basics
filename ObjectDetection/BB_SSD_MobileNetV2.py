...
def load_model(input_shape):
    return tf.saved_model.load('./Models/SSD_MobileNetV2_320x320/saved_model')
...
model, base_model = load_model(input_shape)
...
model = tf.saved_model.load('./Models/SSD_MobileNetV2_320x320/saved_model')

# %%
image = np.array(Image.open('./BB_Faces/images/0.jpg'))
image = tf.reshape(image, shape=(1,)+image.shape)
model_fn = model.signatures['serving_default']
output_dict = model_fn(image)
print(output_dict)